...
APK="$APPROOT/app/build/outputs/apk/debug/app-debug.apk"; test -s "$APK"; cp "$APK" "$OUT/Huiell-Offline-Voice-S22.apk"
SIZE=$(du -h "$OUT/Huiell-Offline-Voice-S22.apk"|awk '{print $1}'); SHA=$(sha256sum "$OUT/Huiell-Offline-Voice-S22.apk"|awk '{print $1}')
cat > "$OUT/index.html" <<EOF
<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Huiell APK</title><body style="font-family:sans-serif;max-width:760px;margin:40px auto;padding:20px"><h1>Huiell Offline Voice</h1><p>Galaxy S22 arm64 build. Qwen runs locally; multilingual Whisper Tiny is bundled for offline Estonian/English speech recognition.</p><p><a href="/Huiell-Offline-Voice-S22.apk" style="font-size:1.3rem">Download Huiell APK</a></p><p>Size: $SIZE</p><p>SHA-256: <code>$SHA</code></p></body>
EOF
